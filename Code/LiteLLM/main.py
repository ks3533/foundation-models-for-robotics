...
from Code.LiteLLM.utils import all_functions, high_level_functions
...
# errors that arise during execution and are returned by the function. If multiple function calls are provided, only the
...
system_prompt = {"role": "system", "content": "You may only use one function call per response and have to wait for "
                                              ...
                                              "Before attempting to call a function, send one message reasoning which"
                                              " step would be useful to achieve your goal. Afterwards, execute the "
                                              "next logical step."}
...
available_functions, tools = high_level_functions(controller)
